  ...
  return (
    ...
    >
      <div className="relative mb-5 group">
        <div className="w-24 h-24 sm:w-28 sm:h-28 p-1 bg-black border border-white/20 rounded-xl overflow-hidden transition-all duration-300 group-hover:border-white/40">
          <img 
            src={avatarUrl} 
        ...
        <div className={`absolute -bottom-2 -right-2 w-6 h-6 rounded-sm border-2 border-black ${statusColors[discord_status]} shadow-[0_0_10px_rgba(0,0,0,0.5)]`} />
      </div>

      <h1 
        className="text-2xl font-bold text-white mb-1 tracking-tight glitch-hover cursor-crosshair"
        data-text={discord_user.global_name || discord_user.username}
      >
        {discord_user.global_name || discord_user.username}
      </h1>
      <p className="text-white/40 font-mono text-xs mb-3">@{discord_user.username}</p>

      {/* LC Sword Badge - Redesigned to match the image */}
      <div className="flex gap-2 mb-5">
        <div className="flex items-center gap-1.5 px-3 py-1 bg-[#111111] border border-white/5 rounded-full shadow-[inset_0_0_10px_rgba(0,0,0,0.5)] hover:bg-[#1a1a1a] transition-colors cursor-crosshair">
          <Sword className="w-3.5 h-3.5 text-[#a87ffb] fill-[#a87ffb]/20" style={{ transform: 'rotate(45deg)' }} />
          <span className="text-[13px] font-bold text-[#e0e0e0] tracking-wide">LC</span>
        </div>
      </div>

      {custom_status && (
        <div className="bg-black/50 border border-white/10 rounded-md px-4 py-2 flex items-center gap-2 mb-3 w-full max-w-[280px] justify-center">
          {custom_status.emoji && (
          ...
          {custom_status.text && (
            <span className="text-white/80 text-sm font-mono truncate">{custom_status.text}</span>
          )}
        </div>
      )}
      ...
      {otherActivities.length > 0 && (
        <div className="w-full flex flex-col gap-2 mb-2">
          {otherActivities.map((activity, idx) => {
            ...
            return (
              <div key={idx} className="bg-black/40 border border-white/10 rounded-lg p-3 flex items-center gap-3 w-full text-left hover:border-white/20 transition-colors">
                {/* Activity Image */}
                <div className="relative w-14 h-14 rounded-md overflow-hidden bg-white/5 flex-shrink-0 flex items-center justify-center">
                  {imageUrl ? (
                  ...
                  {/* Small Image Overlay */}
                  {smallImageUrl && (
                    <div className="absolute -bottom-1 -right-1 w-5 h-5 rounded-full border-2 border-black overflow-hidden bg-black">
                ...
                <div className="flex flex-col flex-1 min-w-0">
                  <span className="text-[10px] font-bold text-white/50 uppercase tracking-wider mb-0.5">
                    {getActivityLabel(activity.type)}
                  </span>
                  <span className="text-sm font-bold text-white truncate">{activity.name}</span>
                  {activity.details && <span className="text-xs text-white/70 truncate font-mono">{activity.details}</span>}
                  {activity.state && <span className="text-xs text-white/70 truncate font-mono">{activity.state}</span>}
                </div>
              </div>
            );
          })}
